// 历代皇帝数据
// 键名对应 dynasties.js 中的 key 字段

export const emperors = {
  Xia: [
    {
      name: '姒文命（禹）',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -2070,
      end: -2025,
      queens: []
    }, // 大禹
    {
      name: '姒启',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -2025,
      end: -2015,
      queens: []
    },
    {
      name: '姒太康',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -2015,
      end: -1986,
      queens: []
    },
    {
      name: '姒仲康',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1986,
      end: -1973,
      queens: []
    },
    {
      name: '姒相',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1973,
      end: -1945,
      queens: []
    },
    {
      name: '姒少康',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1915,
      end: -1894,
      queens: []
    },
    {
      name: '姒杼',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1894,
      end: -1877,
      queens: []
    },
    {
      name: '姒槐',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1877,
      end: -1851,
      queens: []
    },
    {
      name: '姒芒',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1851,
      end: -1833,
      queens: []
    },
    {
      name: '姒泄',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1833,
      end: -1812,
      queens: []
    },
    {
      name: '姒不降',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1812,
      end: -1753,
      queens: []
    },
    {
      name: '姒扃',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1753,
      end: -1732,
      queens: []
    },
    {
      name: '姒廑',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1732,
      end: -1712,
      queens: []
    },
    {
      name: '姒孔甲',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1712,
      end: -1681,
      queens: []
    },
    {
      name: '姒皋',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1681,
      end: -1670,
      queens: []
    },
    {
      name: '姒发',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -1670,
      end: -1651,
      queens: []
    },
    {
      name: '姒履癸',
      templeName: '',
      posthumousName: '桀',
      era: [],
      start: -1651,
      end: -1600,
      queens: []
    }
  ],
  Shang: [
    {
      name: '子履',
      templeName: '商汤',
      posthumousName: '',
      era: [],
      start: -1600,
      end: -1587,
      queens: []
    },
    {
      name: '子至',
      templeName: '太甲',
      posthumousName: '',
      era: [],
      start: -1582,
      end: -1570,
      queens: []
    },
    {
      name: '子怡',
      templeName: '太戊',
      posthumousName: '',
      era: [],
      start: -1480,
      end: -1405,
      queens: []
    },
    {
      name: '子旬',
      templeName: '盘庚',
      posthumousName: '',
      era: [],
      start: -1300,
      end: -1272,
      queens: []
    },
    {
      name: '子昭',
      templeName: '武丁',
      posthumousName: '',
      era: [],
      start: -1250,
      end: -1192,
      queens: []
    },
    {
      name: '子受',
      templeName: '',
      posthumousName: '帝辛',
      era: [],
      start: -1075,
      end: -1046,
      queens: []
    } // 帝辛
  ],
  WesternZhou: [
    {
      name: '姬发',
      templeName: '周武王',
      posthumousName: '武王',
      era: [],
      start: -1046,
      end: -1043,
      queens: []
    },
    {
      name: '姬诵',
      templeName: '周成王',
      posthumousName: '成王',
      era: [],
      start: -1042,
      end: -1021,
      queens: []
    },
    {
      name: '姬钊',
      templeName: '周康王',
      posthumousName: '康王',
      era: [],
      start: -1020,
      end: -996,
      queens: []
    },
    {
      name: '姬瑕',
      templeName: '周昭王',
      posthumousName: '昭王',
      era: [],
      start: -995,
      end: -977,
      queens: []
    },
    {
      name: '姬满',
      templeName: '周穆王',
      posthumousName: '穆王',
      era: [],
      start: -976,
      end: -922,
      queens: []
    },
    {
      name: '姬伊扈',
      templeName: '周共王',
      posthumousName: '共王',
      era: [],
      start: -922,
      end: -900,
      queens: []
    },
    {
      name: '姬囏',
      templeName: '周懿王',
      posthumousName: '懿王',
      era: [],
      start: -899,
      end: -892,
      queens: []
    },
    {
      name: '姬辟方',
      templeName: '周孝王',
      posthumousName: '孝王',
      era: [],
      start: -891,
      end: -886,
      queens: []
    },
    {
      name: '姬燮',
      templeName: '周夷王',
      posthumousName: '夷王',
      era: [],
      start: -885,
      end: -878,
      queens: []
    },
    {
      name: '姬胡',
      templeName: '周厉王',
      posthumousName: '厉王',
      era: [],
      start: -877,
      end: -841,
      queens: []
    },
    {
      name: '共和行政',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -841,
      end: -828,
      queens: []
    },
    {
      name: '姬静',
      templeName: '周宣王',
      posthumousName: '宣王',
      era: [],
      start: -827,
      end: -782,
      queens: []
    },
    {
      name: '姬宫湦',
      templeName: '周幽王',
      posthumousName: '幽王',
      era: [],
      start: -781,
      end: -771,
      queens: []
    }
  ],
  EasternZhou: [
    {
      name: '姬宜臼',
      templeName: '周平王',
      posthumousName: '平王',
      era: [],
      start: -770,
      end: -720,
      queens: []
    },
    {
      name: '姬林',
      templeName: '周桓王',
      posthumousName: '桓王',
      era: [],
      start: -719,
      end: -697,
      queens: []
    },
    {
      name: '姬佗',
      templeName: '周庄王',
      posthumousName: '庄王',
      era: [],
      start: -696,
      end: -682,
      queens: []
    },
    {
      name: '姬胡齐',
      templeName: '周僖王',
      posthumousName: '僖王',
      era: [],
      start: -681,
      end: -677,
      queens: []
    },
    {
      name: '姬阆',
      templeName: '周惠王',
      posthumousName: '惠王',
      era: [],
      start: -676,
      end: -652,
      queens: []
    },
    {
      name: '姬郑',
      templeName: '周襄王',
      posthumousName: '襄王',
      era: [],
      start: -651,
      end: -619,
      queens: []
    },
    {
      name: '姬壬臣',
      templeName: '周顷王',
      posthumousName: '顷王',
      era: [],
      start: -618,
      end: -613,
      queens: []
    },
    {
      name: '姬班',
      templeName: '周匡王',
      posthumousName: '匡王',
      era: [],
      start: -612,
      end: -607,
      queens: []
    },
    {
      name: '姬瑜',
      templeName: '周定王',
      posthumousName: '定王',
      era: [],
      start: -606,
      end: -586,
      queens: []
    },
    {
      name: '姬夷',
      templeName: '周简王',
      posthumousName: '简王',
      era: [],
      start: -585,
      end: -572,
      queens: []
    },
    {
      name: '姬泄心',
      templeName: '周灵王',
      posthumousName: '灵王',
      era: [],
      start: -571,
      end: -545,
      queens: []
    },
    {
      name: '姬贵',
      templeName: '周景王',
      posthumousName: '景王',
      era: [],
      start: -544,
      end: -520,
      queens: []
    },
    {
      name: '姬猛',
      templeName: '周悼王',
      posthumousName: '悼王',
      era: [],
      start: -520,
      end: -520,
      queens: []
    },
    {
      name: '姬丐',
      templeName: '周敬王',
      posthumousName: '敬王',
      era: [],
      start: -519,
      end: -477,
      queens: []
    },
    {
      name: '姬仁',
      templeName: '周元王',
      posthumousName: '元王',
      era: [],
      start: -476,
      end: -469,
      queens: []
    },
    {
      name: '姬介',
      templeName: '周贞定王',
      posthumousName: '贞定王',
      era: [],
      start: -468,
      end: -441,
      queens: []
    },
    {
      name: '姬去疾',
      templeName: '周哀王',
      posthumousName: '哀王',
      era: [],
      start: -441,
      end: -441,
      queens: []
    },
    {
      name: '姬叔',
      templeName: '周思王',
      posthumousName: '思王',
      era: [],
      start: -441,
      end: -441,
      queens: []
    },
    {
      name: '姬嵬',
      templeName: '周考王',
      posthumousName: '考王',
      era: [],
      start: -440,
      end: -426,
      queens: []
    },
    {
      name: '姬午',
      templeName: '周威烈王',
      posthumousName: '威烈王',
      era: [],
      start: -425,
      end: -402,
      queens: []
    },
    {
      name: '姬骄',
      templeName: '周安王',
      posthumousName: '安王',
      era: [],
      start: -401,
      end: -376,
      queens: []
    },
    {
      name: '姬喜',
      templeName: '周烈王',
      posthumousName: '烈王',
      era: [],
      start: -375,
      end: -369,
      queens: []
    },
    {
      name: '姬扁',
      templeName: '周显王',
      posthumousName: '显王',
      era: [],
      start: -368,
      end: -321,
      queens: []
    },
    {
      name: '姬定',
      templeName: '周慎靓王',
      posthumousName: '慎靓王',
      era: [],
      start: -320,
      end: -315,
      queens: []
    },
    {
      name: '姬延',
      templeName: '周赧王',
      posthumousName: '赧王',
      era: [],
      start: -314,
      end: -256,
      queens: []
    }
  ],
  QinGuo: [
    {
      name: '秦非子',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -900,
      end: -858,
      queens: []
    },
    {
      name: '秦侯',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -857,
      end: -848,
      queens: []
    },
    {
      name: '秦公伯',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -847,
      end: -845,
      queens: []
    },
    {
      name: '秦仲',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -844,
      end: -822,
      queens: []
    },
    {
      name: '秦庄公',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -821,
      end: -778,
      queens: []
    },
    {
      name: '秦襄公',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -777,
      end: -766,
      queens: []
    },
    {
      name: '秦文公',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -765,
      end: -716,
      queens: []
    },
    {
      name: '秦宪公',
      templeName: '',
      posthumousName: '宪',
      era: [],
      start: -715,
      end: -704,
      queens: []
    },
    {
      name: '秦出子',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -703,
      end: -698,
      queens: []
    },
    {
      name: '秦武公',
      templeName: '',
      posthumousName: '武',
      era: [],
      start: -697,
      end: -678,
      queens: []
    },
    {
      name: '秦德公',
      templeName: '',
      posthumousName: '德',
      era: [],
      start: -677,
      end: -676,
      queens: []
    },
    {
      name: '秦宣公',
      templeName: '',
      posthumousName: '宣',
      era: [],
      start: -675,
      end: -664,
      queens: []
    },
    {
      name: '秦成公',
      templeName: '',
      posthumousName: '成',
      era: [],
      start: -663,
      end: -660,
      queens: []
    },
    {
      name: '秦穆公',
      templeName: '',
      posthumousName: '穆',
      era: [],
      start: -659,
      end: -621,
      queens: []
    },
    {
      name: '秦康公',
      templeName: '',
      posthumousName: '康',
      era: [],
      start: -620,
      end: -609,
      queens: []
    },
    {
      name: '秦共公',
      templeName: '',
      posthumousName: '共',
      era: [],
      start: -608,
      end: -604,
      queens: []
    },
    {
      name: '秦桓公',
      templeName: '',
      posthumousName: '桓',
      era: [],
      start: -603,
      end: -577,
      queens: []
    },
    {
      name: '秦景公',
      templeName: '',
      posthumousName: '景',
      era: [],
      start: -576,
      end: -537,
      queens: []
    },
    {
      name: '秦哀公',
      templeName: '',
      posthumousName: '哀',
      era: [],
      start: -536,
      end: -501,
      queens: []
    },
    {
      name: '秦惠公',
      templeName: '',
      posthumousName: '惠',
      era: [],
      start: -500,
      end: -491,
      queens: []
    },
    {
      name: '秦悼公',
      templeName: '',
      posthumousName: '悼',
      era: [],
      start: -490,
      end: -477,
      queens: []
    },
    {
      name: '秦厉共公',
      templeName: '',
      posthumousName: '厉共',
      era: [],
      start: -476,
      end: -443,
      queens: []
    },
    {
      name: '秦躁公',
      templeName: '',
      posthumousName: '躁',
      era: [],
      start: -442,
      end: -429,
      queens: []
    },
    {
      name: '秦怀公',
      templeName: '',
      posthumousName: '怀',
      era: [],
      start: -428,
      end: -425,
      queens: []
    },
    {
      name: '秦灵公',
      templeName: '',
      posthumousName: '灵',
      era: [],
      start: -424,
      end: -415,
      queens: []
    },
    {
      name: '秦简公',
      templeName: '',
      posthumousName: '简',
      era: [],
      start: -414,
      end: -400,
      queens: []
    },
    {
      name: '秦惠公',
      templeName: '',
      posthumousName: '惠',
      era: [],
      start: -399,
      end: -387,
      queens: []
    },
    {
      name: '秦出子',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -386,
      end: -385,
      queens: []
    },
    {
      name: '秦献公',
      templeName: '',
      posthumousName: '献',
      era: [],
      start: -384,
      end: -362,
      queens: []
    },
    {
      name: '秦孝公',
      templeName: '',
      posthumousName: '孝',
      era: [],
      start: -361,
      end: -338,
      queens: []
    },
    {
      name: '秦惠文王',
      templeName: '',
      posthumousName: '惠文',
      era: [],
      start: -337,
      end: -311,
      queens: []
    },
    {
      name: '秦武王',
      templeName: '',
      posthumousName: '武',
      era: [],
      start: -310,
      end: -307,
      queens: []
    },
    {
      name: '秦昭襄王',
      templeName: '',
      posthumousName: '昭襄',
      era: [],
      start: -306,
      end: -251,
      queens: []
    },
    {
      name: '秦孝文王',
      templeName: '',
      posthumousName: '孝文',
      era: [],
      start: -250,
      end: -250,
      queens: []
    },
    {
      name: '秦庄襄王',
      templeName: '',
      posthumousName: '庄襄',
      era: [],
      start: -249,
      end: -247,
      queens: []
    },
    {
      name: '秦王政',
      templeName: '',
      posthumousName: '',
      era: [],
      start: -246,
      end: -221,
      queens: []
    }
  ],
  Qin: [
    {
      name: '嬴政',
      templeName: '秦始皇',
      posthumousName: '',
      era: [],
      start: '-221-01-01',
      end: '-210-09-10',
      queens: []
    },
    {
      name: '嬴胡亥',
      templeName: '秦二世',
      posthumousName: '',
      era: [],
      start: '-210-09-10',
      end: '-207-10-01',
      queens: []
    },
    {
      name: '嬴子婴',
      templeName: '',
      posthumousName: '',
      era: [],
      start: '-207-10-01',
      end: '-206-01-01',
      queens: []
    }
  ],
  WesternHan: [
    {
      name: '刘邦',
      templeName: '汉太祖',
      posthumousName: '高皇帝',
      era: [],
      start: '-202-02-28',
      end: '-195-06-01',
      queens: [
        {
          name: '吕雉',
          posthumousName: '高皇后',
          start: '-202-02-28',
          end: '-180-08-18'
        }
      ]
    },
    {
      name: '刘盈',
      templeName: '',
      posthumousName: '孝惠皇帝',
      era: [],
      start: '-195-06-23',
      end: '-188-09-26',
      queens: [
        {
          name: '张嫣',
          posthumousName: '孝惠皇后',
          start: '-192-11',
          end: '-163-04'
        }
      ]
    },
    {
      name: '吕雉',
      templeName: '',
      posthumousName: '高皇后',
      era: [],
      start: '-188-09-26',
      end: '-180-08-18',
      queens: []
    },
    {
      name: '刘恒',
      templeName: '汉太宗',
      posthumousName: '孝文皇帝',
      era: ['前元', '后元'],
      start: '-180-11-14',
      end: '-157-07-06',
      queens: [
        {
          name: '窦氏',
          posthumousName: '孝文皇后',
          start: '-179-03',
          end: '-135-06'
        }
      ]
    },
    {
      name: '刘启',
      templeName: '',
      posthumousName: '孝景皇帝',
      era: ['前元', '中元', '后元'],
      start: '-157-07-14',
      end: '-141-03-09',
      queens: [
        { name: '薄氏', posthumousName: '', start: '-157-07', end: '-151-09' },
        {
          name: '王娡',
          posthumousName: '孝景皇后',
          start: '-150-05',
          end: '-126-06'
        }
      ]
    },
    {
      name: '刘彻',
      templeName: '汉世宗',
      posthumousName: '孝武皇帝',
      era: [
        '建元',
        '元光',
        '元朔',
        '元狩',
        '元鼎',
        '元封',
        '太初',
        '元封',
        '太初',
        '天汉',
        '太始',
        '征和',
        '后元'
      ],
      start: '-141-03-09',
      end: '-87-03-29',
      queens: [
        {
          name: '陈阿娇',
          posthumousName: '',
          start: '-141-04',
          end: '-130-07'
        },
        {
          name: '卫子夫',
          posthumousName: '思皇后',
          start: '-128-04',
          end: '-91-09'
        }
      ]
    },
    {
      name: '刘弗陵',
      templeName: '',
      posthumousName: '孝昭皇帝',
      era: ['始元', '元凤', '元平'],
      start: '-87-03-30',
      end: '-74-06-05',
      queens: [
        {
          name: '上官氏',
          posthumousName: '孝昭皇后',
          start: '-83-04',
          end: '-37-06'
        }
      ]
    },
    {
      name: '刘询',
      templeName: '汉中宗',
      posthumousName: '孝宣皇帝',
      era: ['本始', '地节', '元康', '神爵', '五凤', '甘露', '黄龙'],
      start: '-74-09-10',
      end: '-48-01-10',
      queens: [
        {
          name: '许平君',
          posthumousName: '恭哀皇后',
          start: '-74-12',
          end: '-71-03'
        },
        { name: '霍成君', posthumousName: '', start: '-70-04', end: '-66-09' },
        {
          name: '王氏',
          posthumousName: '孝宣皇后',
          start: '-64-03',
          end: '-16-09'
        }
      ]
    },
    {
      name: '刘奭',
      templeName: '汉元帝',
      posthumousName: '孝元皇帝',
      era: ['初元', '永光', '建昭', '竟宁'],
      start: '-48-01-10',
      end: '-33-07-08',
      queens: [
        {
          name: '王政君',
          posthumousName: '孝元皇后',
          start: '-48-02',
          end: '13-02'
        }
      ]
    },
    {
      name: '刘骜',
      templeName: '汉成帝',
      posthumousName: '孝成皇帝',
      era: ['建始', '河平', '阳朔', '鸿嘉', '永始', '元延', '绥和'],
      start: '-33-07-10',
      end: '-7-04-17',
      queens: [
        { name: '许氏', posthumousName: '孝成皇后', start: '-31', end: '-18' },
        { name: '赵飞燕', posthumousName: '孝成皇后', start: '-16', end: '-1' }
      ]
    },
    {
      name: '刘欣',
      templeName: '汉哀帝',
      posthumousName: '孝哀皇帝',
      era: ['建平', '太初元将', '元寿'],
      start: '-7-05-07',
      end: '-1-08-15',
      queens: [
        { name: '傅氏', posthumousName: '孝哀皇后', start: '-6', end: '-1' }
      ]
    },
    {
      name: '刘衎',
      templeName: '汉平帝',
      posthumousName: '孝平皇帝',
      era: ['元始'],
      start: '-1-10-17',
      end: '6-02-03',
      queens: [
        { name: '王氏', posthumousName: '孝平皇后', start: '4', end: '23' }
      ]
    },
    {
      name: '刘婴',
      templeName: '',
      posthumousName: '',
      era: ['居摄', '初始'],
      start: '6-02-03',
      end: '9-01-10',
      queens: []
    }
  ],
  Xin: [
    {
      name: '王莽',
      templeName: '',
      posthumousName: '',
      era: ['始建国', '天凤', '地皇'],
      start: '9-01-10',
      end: '23-10-06',
      queens: []
    }
  ],
  EasternHan: [
    {
      name: '刘秀',
      templeName: '汉世祖',
      posthumousName: '光武皇帝',
      era: ['建武', '建武中元'],
      start: '25-08-05',
      end: '57-03-29',
      queens: [
        { name: '郭圣通', posthumousName: '', start: '26', end: '41' },
        { name: '阴丽华', posthumousName: '光烈皇后', start: '41', end: '64' }
      ]
    },
    {
      name: '刘庄',
      templeName: '汉显宗',
      posthumousName: '孝明皇帝',
      era: ['永平'],
      start: '57-03-29',
      end: '75-09-05',
      queens: [
        { name: '马氏', posthumousName: '明德皇后', start: '60', end: '79' }
      ]
    },
    {
      name: '刘炟',
      templeName: '汉肃宗',
      posthumousName: '孝章皇帝',
      era: ['建初', '元和', '章和'],
      start: '75-09-05',
      end: '88-04-09',
      queens: [
        { name: '窦氏', posthumousName: '章德皇后', start: '78', end: '97' }
      ]
    },
    {
      name: '刘肇',
      templeName: '汉穆宗',
      posthumousName: '孝和皇帝',
      era: ['永元', '元兴'],
      start: '88-04-09',
      end: '106-02-13',
      queens: [
        { name: '阴氏', posthumousName: '', start: '96', end: '102' },
        { name: '邓绥', posthumousName: '和熹皇后', start: '102', end: '121' }
      ]
    },
    {
      name: '刘隆',
      templeName: '',
      posthumousName: '孝殇皇帝',
      era: ['延平'],
      start: '106-02-13',
      end: '106-09-21',
      queens: []
    },
    {
      name: '刘祜',
      templeName: '汉恭宗',
      posthumousName: '孝安皇帝',
      era: ['永初', '元初', '永宁', '建光', '延光'],
      start: '106-09-21',
      end: '125-04-30',
      queens: [
        { name: '阎姬', posthumousName: '安思皇后', start: '115', end: '126' }
      ]
    },
    {
      name: '刘保',
      templeName: '汉敬宗',
      posthumousName: '孝顺皇帝',
      era: ['永建', '阳嘉', '永和', '汉安', '建康'],
      start: '125-12-16',
      end: '144-09-20',
      queens: [
        { name: '梁纳', posthumousName: '顺烈皇后', start: '132', end: '150' }
      ]
    },
    {
      name: '刘炳',
      templeName: '',
      posthumousName: '孝冲皇帝',
      era: ['永嘉'],
      start: '144-09-20',
      end: '145-02-15',
      queens: []
    },
    {
      name: '刘缵',
      templeName: '',
      posthumousName: '孝质皇帝',
      era: ['本初'],
      start: '145-03-06',
      end: '146-07-26',
      queens: []
    },
    {
      name: '刘志',
      templeName: '汉威宗',
      posthumousName: '孝桓皇帝',
      era: ['建和', '和平', '元嘉', '永兴', '永寿', '延熹', '永康'],
      start: '146-08-01',
      end: '168-01-25',
      queens: [
        {
          name: '梁女莹',
          posthumousName: '懿献皇后',
          start: '147',
          end: '159'
        },
        { name: '邓猛女', posthumousName: '', start: '159', end: '165' },
        { name: '窦妙', posthumousName: '桓思皇后', start: '165', end: '172' }
      ]
    },
    {
      name: '刘宏',
      templeName: '',
      posthumousName: '孝灵皇帝',
      era: ['建宁', '熹平', '光和', '中平'],
      start: '168-02-17',
      end: '189-05-13',
      queens: [
        { name: '宋氏', posthumousName: '', start: '171', end: '178' },
        { name: '何氏', posthumousName: '灵思皇后', start: '180', end: '189' }
      ]
    },
    {
      name: '刘协',
      templeName: '',
      posthumousName: '孝献皇帝',
      era: [
        '初平',
        '光熹',
        '昭宁',
        '永汉',
        '中平',
        '初平',
        '兴平',
        '建安',
        '延康'
      ],
      start: '189-09-28',
      end: '220-12-11',
      queens: [
        { name: '伏寿', posthumousName: '', start: '195', end: '214' },
        { name: '曹节', posthumousName: '献穆皇后', start: '215', end: '260' }
      ]
    }
  ],
  CaoWei: [
    {
      name: '曹丕',
      templeName: '魏世祖',
      posthumousName: '文皇帝',
      era: ['黄初'],
      start: 220,
      end: 226,
      queens: [
        { name: '郭女王', posthumousName: '文德皇后', start: '222', end: '235' }
      ]
    },
    {
      name: '曹叡',
      templeName: '魏烈祖',
      posthumousName: '明皇帝',
      era: ['太和', '青龙', '景初'],
      start: 226,
      end: 239,
      queens: [
        { name: '毛氏', posthumousName: '明悼皇后', start: '227', end: '237' },
        { name: '郭氏', posthumousName: '明元皇后', start: '239', end: '264' }
      ]
    },
    {
      name: '曹芳',
      templeName: '',
      posthumousName: '邵陵厉公',
      era: ['正始', '嘉平'],
      start: 239,
      end: 254,
      queens: [
        { name: '甄氏', posthumousName: '怀皇后', start: '243', end: '251' },
        { name: '张氏', posthumousName: '', start: '252', end: '254' },
        { name: '王氏', posthumousName: '', start: '254', end: '254' }
      ]
    },
    {
      name: '曹髦',
      templeName: '',
      posthumousName: '高贵乡公',
      era: ['正元', '甘露'],
      start: 254,
      end: 260,
      queens: [{ name: '卞氏', posthumousName: '', start: '255', end: '260' }]
    },
    {
      name: '曹奂',
      templeName: '',
      posthumousName: '元皇帝',
      era: ['景元', '咸熙'],
      start: 260,
      end: 265,
      queens: [{ name: '卞氏', posthumousName: '', start: '263', end: '265' }]
    }
  ],
  ShuHan: [
    {
      name: '刘备',
      templeName: '汉烈祖',
      posthumousName: '昭烈皇帝',
      era: ['章武'],
      start: 221,
      end: 223,
      queens: [
        { name: '吴氏', posthumousName: '穆皇后', start: '221', end: '245' }
      ]
    },
    {
      name: '刘禅',
      templeName: '',
      posthumousName: '孝怀皇帝',
      era: ['建兴', '延熙', '景耀', '炎兴'],
      start: 223,
      end: 263,
      queens: [
        { name: '张氏', posthumousName: '敬哀皇后', start: '223', end: '237' },
        { name: '张氏', posthumousName: '', start: '238', end: '263' }
      ]
    }
  ],
  EasternWu: [
    {
      name: '孙权',
      templeName: '吴太祖',
      posthumousName: '大皇帝',
      era: ['黄武', '黄龙', '嘉禾', '赤乌', '太元', '神凤'],
      start: 229,
      end: 252,
      queens: [
        { name: '潘氏', posthumousName: '潘皇后', start: '251', end: '252' }
      ]
    },
    {
      name: '孙亮',
      templeName: '',
      posthumousName: '会稽王',
      era: ['建兴', '五凤', '太平'],
      start: 252,
      end: 258,
      queens: [
        { name: '全氏', posthumousName: '全皇后', start: '253', end: '258' }
      ]
    },
    {
      name: '孙休',
      templeName: '吴太宗',
      posthumousName: '景皇帝',
      era: ['永安'],
      start: 258,
      end: 264,
      queens: [
        { name: '朱氏', posthumousName: '朱皇后', start: '262', end: '265' }
      ]
    },
    {
      name: '孙皓',
      templeName: '',
      posthumousName: '归命侯',
      era: ['元兴', '甘露', '宝鼎', '建衡', '凤凰', '天册', '天玺', '天纪'],
      start: 264,
      end: 280,
      queens: [
        { name: '滕氏', posthumousName: '滕皇后', start: '264', end: '280' }
      ]
    }
  ],
  WesternJin: [
    {
      name: '司马炎',
      templeName: '晋世祖',
      posthumousName: '武皇帝',
      era: ['泰始', '咸宁', '太康', '太熙'],
      start: 266,
      end: 290,
      queens: [
        { name: '杨艳', posthumousName: '武元皇后', start: 266, end: 274 },
        { name: '杨芷', posthumousName: '武悼皇后', start: 276, end: 290 }
      ]
    },
    {
      name: '司马衷',
      templeName: '',
      posthumousName: '惠皇帝',
      era: ['永熙', '永平', '元康', '永康', '永宁', '太安', '永安', '建武', '永兴', '光熙'],
      start: 290,
      end: 306,
      queens: [
        { name: '贾南风', posthumousName: '惠贾皇后', start: 290, end: 300 },
        { name: '羊献容', posthumousName: '惠羊皇后', start: 300, end: 306 }
      ]
    },
    {
      name: '司马炽',
      templeName: '',
      posthumousName: '怀皇帝',
      era: ['永嘉'],
      start: 306,
      end: 311,
      queens: []
    },
    {
      name: '司马邺',
      templeName: '',
      posthumousName: '愍皇帝',
      era: ['建兴'],
      start: 313,
      end: 316,
      queens: []
    }
  ],
  EasternJin: [
    {
      name: '司马睿',
      templeName: '晋中宗',
      posthumousName: '元皇帝',
      era: ['建武', '太兴', '永昌'],
      start: 317,
      end: 323,
      queens: [
        { name: '虞孟母', posthumousName: '元敬皇后', start: '317', end: '323' }
      ]
    },
    {
      name: '司马绍',
      templeName: '晋肃宗',
      posthumousName: '明皇帝',
      era: ['太宁'],
      start: 323,
      end: 325,
      queens: [
        { name: '庾文君', posthumousName: '明穆皇后', start: '323', end: '325' }
      ]
    },
    {
      name: '司马衍',
      templeName: '晋显宗',
      posthumousName: '成皇帝',
      era: ['咸和', '咸康'],
      start: 325,
      end: 342,
      queens: [
        { name: '杜陵阳', posthumousName: '成恭皇后', start: '336', end: '342' }
      ]
    },
    {
      name: '司马岳',
      templeName: '晋康宗',
      posthumousName: '康皇帝',
      era: ['建元'],
      start: 342,
      end: 344,
      queens: [
        { name: '褚蒜子', posthumousName: '康献皇后', start: '342', end: '344' }
      ]
    },
    {
      name: '司马聃',
      templeName: '晋孝宗',
      posthumousName: '穆皇帝',
      era: ['永和', '升平'],
      start: 344,
      end: 361,
      queens: [
        { name: '何法倪', posthumousName: '穆章皇后', start: '357', end: '361' }
      ]
    },
    {
      name: '司马丕',
      templeName: '晋哀宗',
      posthumousName: '哀皇帝',
      era: ['隆和', '兴宁'],
      start: 361,
      end: 365,
      queens: [
        { name: '王穆之', posthumousName: '哀靖皇后', start: '361', end: '364' }
      ]
    },
    {
      name: '司马奕',
      templeName: '',
      posthumousName: '废皇帝',
      era: ['太和'],
      start: 365,
      end: 371,
      queens: [
        { name: '庾道怜', posthumousName: '孝皇后', start: '365', end: '371' }
      ]
    },
    {
      name: '司马昱',
      templeName: '晋太宗',
      posthumousName: '简文皇帝',
      era: ['咸安'],
      start: 371,
      end: 372,
      queens: [
        { name: '王简姬', posthumousName: '简文顺皇后', start: '371', end: '372' }
      ]
    },
    {
      name: '司马曜',
      templeName: '晋烈宗',
      posthumousName: '孝武皇帝',
      era: ['宁康', '太元'],
      start: 372,
      end: 396,
      queens: [
        { name: '王法慧', posthumousName: '孝武定皇后', start: '381', end: '389' }
      ]
    },
    {
      name: '司马德宗',
      templeName: '晋安帝',
      posthumousName: '安皇帝',
      era: ['隆安', '元兴', '义熙'],
      start: 396,
      end: 418,
      queens: [
        { name: '王神爱', posthumousName: '安僖皇后', start: '396', end: '412' }
      ]
    },
    {
      name: '司马德文',
      templeName: '晋恭帝',
      posthumousName: '恭皇帝',
      era: ['元熙'],
      start: 418,
      end: 420,
      queens: [
        { name: '褚灵媛', posthumousName: '恭思皇后', start: '418', end: '420' }
      ]
    }
  ],
  LiuSong: [
    {
      name: '刘裕',
      templeName: '宋高祖',
      posthumousName: '武皇帝',
      era: ['永初'],
      start: 420,
      end: 422,
      queens: []
    },
    {
      name: '刘义符',
      templeName: '',
      posthumousName: '少帝',
      era: ['景平'],
      start: 422,
      end: 424,
      queens: []
    },
    {
      name: '刘义隆',
      templeName: '宋太祖',
      posthumousName: '文皇帝',
      era: ['元嘉'],
      start: 424,
      end: 453,
      queens: []
    },
    {
      name: '刘骏',
      templeName: '宋世祖',
      posthumousName: '孝武皇帝',
      era: ['孝建', '大明'],
      start: 453,
      end: 464,
      queens: []
    },
    {
      name: '刘子业',
      templeName: '',
      posthumousName: '前废帝',
      era: ['永光', '景和'],
      start: 464,
      end: 465,
      queens: []
    },
    {
      name: '刘彧',
      templeName: '宋太宗',
      posthumousName: '明皇帝',
      era: ['泰始', '泰豫'],
      start: 465,
      end: 472,
      queens: []
    },
    {
      name: '刘昱',
      templeName: '',
      posthumousName: '后废帝',
      era: ['元徽'],
      start: 472,
      end: 477,
      queens: []
    },
    {
      name: '刘准',
      templeName: '',
      posthumousName: '顺皇帝',
      era: ['升明'],
      start: 477,
      end: 479,
      queens: []
    }
  ],
  SouthernQi: [
    {
      name: '萧道成',
      templeName: '齐太祖',
      posthumousName: '高皇帝',
      era: ['建元'],
      start: 479,
      end: 482,
      queens: []
    },
    {
      name: '萧赜',
      templeName: '齐世祖',
      posthumousName: '武皇帝',
      era: ['永明'],
      start: 482,
      end: 493,
      queens: []
    },
    {
      name: '萧昭业',
      templeName: '',
      posthumousName: '郁林王',
      era: ['隆昌'],
      start: 493,
      end: 494,
      queens: []
    },
    {
      name: '萧昭文',
      templeName: '',
      posthumousName: '海陵王',
      era: ['延兴'],
      start: 494,
      end: 494,
      queens: []
    },
    {
      name: '萧鸾',
      templeName: '齐高宗',
      posthumousName: '明皇帝',
      era: ['建武', '永泰'],
      start: 494,
      end: 498,
      queens: []
    },
    {
      name: '萧宝卷',
      templeName: '',
      posthumousName: '东昏侯',
      era: ['永元'],
      start: 498,
      end: 501,
      queens: []
    },
    {
      name: '萧宝融',
      templeName: '',
      posthumousName: '和皇帝',
      era: ['中兴'],
      start: 501,
      end: 502,
      queens: []
    }
  ],
  Liang: [
    {
      name: '萧衍',
      templeName: '梁高祖',
      posthumousName: '武皇帝',
      era: ['天监', '普通', '大通', '中大通', '大同', '中大同', '太清'],
      start: 502,
      end: 549,
      queens: []
    },
    {
      name: '萧纲',
      templeName: '梁太宗',
      posthumousName: '简文皇帝',
      era: ['大宝'],
      start: 549,
      end: 551,
      queens: []
    },
    {
      name: '萧绎',
      templeName: '梁世祖',
      posthumousName: '元皇帝',
      era: ['承圣'],
      start: 552,
      end: 555,
      queens: []
    },
    {
      name: '萧方智',
      templeName: '',
      posthumousName: '敬皇帝',
      era: ['绍泰', '太平'],
      start: 555,
      end: 557,
      queens: []
    }
  ],
  Chen: [
    {
      name: '陈霸先',
      templeName: '陈高祖',
      posthumousName: '武皇帝',
      era: ['永定'],
      start: 557,
      end: 559,
      queens: []
    },
    {
      name: '陈蒨',
      templeName: '陈世祖',
      posthumousName: '文皇帝',
      era: ['天嘉', '天康'],
      start: 559,
      end: 566,
      queens: []
    },
    {
      name: '陈伯宗',
      templeName: '',
      posthumousName: '废帝',
      era: ['光大'],
      start: 566,
      end: 568,
      queens: []
    },
    {
      name: '陈顼',
      templeName: '陈高宗',
      posthumousName: '宣皇帝',
      era: ['太建'],
      start: 568,
      end: 582,
      queens: []
    },
    {
      name: '陈叔宝',
      templeName: '',
      posthumousName: '后主',
      era: ['至德', '祯明'],
      start: 582,
      end: 589,
      queens: []
    }
  ],
  NorthernWei: [
    {
      name: '拓跋珪',
      templeName: '魏太祖',
      posthumousName: '道武皇帝',
      era: ['登国', '皇始', '天兴', '天赐'],
      start: 386,
      end: 409,
      queens: []
    },
    {
      name: '拓跋嗣',
      templeName: '魏太宗',
      posthumousName: '明元皇帝',
      era: ['永兴', '神瑞', '泰常'],
      start: 409,
      end: 423,
      queens: []
    },
    {
      name: '拓跋焘',
      templeName: '魏世祖',
      posthumousName: '太武皇帝',
      era: ['始光', '神麚', '延和', '太延', '太平真君', '正平'],
      start: 423,
      end: 452,
      queens: []
    },
    {
      name: '拓跋浚',
      templeName: '魏高宗',
      posthumousName: '文成皇帝',
      era: ['兴安', '兴光', '太安', '和平'],
      start: 452,
      end: 465,
      queens: []
    },
    {
      name: '拓跋弘',
      templeName: '魏显祖',
      posthumousName: '献文皇帝',
      era: ['天安', '皇兴'],
      start: 465,
      end: 471,
      queens: []
    },
    {
      name: '元宏',
      templeName: '魏高祖',
      posthumousName: '孝文皇帝',
      era: ['延兴', '承明', '太和'],
      start: 471,
      end: 499,
      queens: []
    },
    {
      name: '元恪',
      templeName: '魏世宗',
      posthumousName: '宣武皇帝',
      era: ['景明', '正始', '永平', '延昌'],
      start: 499,
      end: 515,
      queens: []
    },
    {
      name: '元诩',
      templeName: '魏肃宗',
      posthumousName: '孝明皇帝',
      era: ['熙平', '神龟', '正光', '孝昌', '武泰'],
      start: 515,
      end: 528,
      queens: []
    },
    {
      name: '元子攸',
      templeName: '魏敬宗',
      posthumousName: '孝庄皇帝',
      era: ['建义', '永安'],
      start: 528,
      end: 530,
      queens: []
    },
    {
      name: '元修',
      templeName: '',
      posthumousName: '孝武皇帝',
      era: ['太昌', '永兴', '永熙'],
      start: 532,
      end: 534,
      queens: []
    }
  ],
  EasternWei: [
    {
      name: '元善见',
      templeName: '',
      posthumousName: '孝静皇帝',
      era: ['天平', '元象', '兴和', '武定'],
      start: 534,
      end: 550,
      queens: []
    }
  ],
  WesternWei: [
    {
      name: '元宝炬',
      templeName: '',
      posthumousName: '文皇帝',
      era: ['大统'],
      start: 535,
      end: 551,
      queens: []
    },
    {
      name: '元钦',
      templeName: '',
      posthumousName: '废帝',
      era: [],
      start: 551,
      end: 554,
      queens: []
    },
    {
      name: '元廓',
      templeName: '',
      posthumousName: '恭皇帝',
      era: [],
      start: 554,
      end: 557,
      queens: []
    }
  ],
  NorthernQi: [
    {
      name: '高洋',
      templeName: '齐显祖',
      posthumousName: '文宣皇帝',
      era: ['天保'],
      start: 550,
      end: 559,
      queens: []
    },
    {
      name: '高殷',
      templeName: '',
      posthumousName: '废帝',
      era: ['乾明'],
      start: 559,
      end: 560,
      queens: []
    },
    {
      name: '高演',
      templeName: '齐肃宗',
      posthumousName: '孝昭皇帝',
      era: ['皇建'],
      start: 560,
      end: 561,
      queens: []
    },
    {
      name: '高湛',
      templeName: '齐世祖',
      posthumousName: '武成皇帝',
      era: ['太宁', '河清'],
      start: 561,
      end: 565,
      queens: []
    },
    {
      name: '高纬',
      templeName: '',
      posthumousName: '后主',
      era: ['天统', '武平', '隆化'],
      start: 565,
      end: 577,
      queens: []
    },
    {
      name: '高恒',
      templeName: '',
      posthumousName: '幼主',
      era: ['承光'],
      start: 577,
      end: 577,
      queens: []
    }
  ],
  NorthernZhou: [
    {
      name: '宇文觉',
      templeName: '',
      posthumousName: '孝闵皇帝',
      era: [],
      start: 557,
      end: 557,
      queens: []
    },
    {
      name: '宇文毓',
      templeName: '周世宗',
      posthumousName: '明皇帝',
      era: ['武成', '保定'],
      start: 557,
      end: 560,
      queens: []
    },
    {
      name: '宇文邕',
      templeName: '周高祖',
      posthumousName: '武皇帝',
      era: ['天和', '建德', '宣政'],
      start: 560,
      end: 578,
      queens: []
    },
    {
      name: '宇文赟',
      templeName: '',
      posthumousName: '宣皇帝',
      era: ['大成'],
      start: 578,
      end: 579,
      queens: []
    },
    {
      name: '宇文阐',
      templeName: '',
      posthumousName: '静皇帝',
      era: ['大象', '大定'],
      start: 579,
      end: 581,
      queens: []
    }
  ],
  LaterLiang: [
    {
      name: '朱温',
      templeName: '梁太祖',
      posthumousName: '神武元圣孝皇帝',
      era: ['开平', '乾化'],
      start: 907,
      end: 912,
      queens: []
    },
    {
      name: '朱友珪',
      templeName: '',
      posthumousName: '',
      era: ['乾化', '凤历'],
      start: 912,
      end: 913,
      queens: []
    },
    {
      name: '朱友贞',
      templeName: '',
      posthumousName: '末帝',
      era: ['乾化', '贞明', '龙德'],
      start: 913,
      end: 923,
      queens: []
    }
  ],
  LaterTang: [
    {
      name: '李存勖',
      templeName: '唐庄宗',
      posthumousName: '光圣神闵孝皇帝',
      era: ['同光'],
      start: 923,
      end: 926,
      queens: []
    },
    {
      name: '李嗣源',
      templeName: '唐明宗',
      posthumousName: '圣德和武钦孝皇帝',
      era: ['天成', '长兴'],
      start: 926,
      end: 933,
      queens: []
    },
    {
      name: '李从厚',
      templeName: '',
      posthumousName: '闵皇帝',
      era: ['应顺'],
      start: 933,
      end: 934,
      queens: []
    },
    {
      name: '李从珂',
      templeName: '',
      posthumousName: '末帝',
      era: ['清泰'],
      start: 934,
      end: 936,
      queens: []
    }
  ],
  LaterJin: [
    {
      name: '石敬瑭',
      templeName: '晋高祖',
      posthumousName: '圣文章武明德孝皇帝',
      era: ['天福'],
      start: 936,
      end: 942,
      queens: []
    },
    {
      name: '石重贵',
      templeName: '',
      posthumousName: '出帝',
      era: ['天福', '开运'],
      start: 942,
      end: 947,
      queens: []
    }
  ],
  LaterHan: [
    {
      name: '刘知远',
      templeName: '汉高祖',
      posthumousName: '睿文圣武昭肃孝皇帝',
      era: ['天福', '乾祐'],
      start: 947,
      end: 948,
      queens: []
    },
    {
      name: '刘承祐',
      templeName: '',
      posthumousName: '隐皇帝',
      era: ['乾祐'],
      start: 948,
      end: 950,
      queens: []
    }
  ],
  LaterZhou: [
    {
      name: '郭威',
      templeName: '周太祖',
      posthumousName: '圣神恭肃文武孝皇帝',
      era: ['广顺', '显德'],
      start: 951,
      end: 954,
      queens: []
    },
    {
      name: '柴荣',
      templeName: '周世宗',
      posthumousName: '睿武孝文皇帝',
      era: ['显德'],
      start: 954,
      end: 959,
      queens: []
    },
    {
      name: '柴宗训',
      templeName: '',
      posthumousName: '恭皇帝',
      era: ['显德'],
      start: 959,
      end: 960,
      queens: []
    }
  ],
  TenKingdoms_Wu: [
    { name: '杨行密', templeName: '吴太祖', start: 902, end: 905 },
    { name: '杨渥', templeName: '吴烈祖', start: 905, end: 908 },
    { name: '杨隆演', templeName: '吴高祖', start: 908, end: 920 },
    { name: '杨溥', templeName: '吴睿帝', start: 920, end: 937 }
  ],
  TenKingdoms_Wuyue: [
    { name: '钱镠', templeName: '吴越太祖', start: 907, end: 932 },
    { name: '钱元瓘', templeName: '吴越世宗', start: 932, end: 941 },
    { name: '钱弘佐', templeName: '吴越成宗', start: 941, end: '947-03' },
    {
      name: '钱弘倧',
      templeName: '吴越忠逊王',
      start: '947-03',
      end: '947-12',
      queens: []
    },
    { name: '钱弘俶', templeName: '吴越忠懿王', start: '947-12', end: 978 }
  ],
  TenKingdoms_Min: [
    { name: '王审知', templeName: '闽太祖', start: 909, end: 925 },
    { name: '王延翰', templeName: '闽嗣王', start: 925, end: 926 },
    { name: '王延钧', templeName: '闽惠宗', start: 926, end: 935 },
    { name: '王继鹏', templeName: '闽康宗', start: 935, end: 939 },
    { name: '王延羲', templeName: '闽景宗', start: 939, end: 944 },
    { name: '王延政', templeName: '闽天德帝', start: 944, end: 945 }
  ],
  TenKingdoms_Chu: [
    { name: '马殷', templeName: '楚武穆王', start: 907, end: 930 },
    { name: '马希声', templeName: '楚衡阳王', start: 930, end: 932 },
    { name: '马希范', templeName: '楚文昭王', start: 932, end: 947 },
    { name: '马希广', templeName: '楚废王', start: 947, end: 950 },
    { name: '马希鄂', templeName: '楚恭孝王', start: 950, end: 951 }
  ],
  TenKingdoms_SouthernHan: [
    { name: '刘龑', templeName: '南汉高祖', start: 917, end: 942 },
    { name: '刘玢', templeName: '南汉殇帝', start: 942, end: 943 },
    { name: '刘晟', templeName: '南汉中宗', start: 943, end: 958 },
    { name: '刘鋹', templeName: '南汉后主', start: 958, end: 971 }
  ],
  TenKingdoms_FormerShu: [
    { name: '王建', templeName: '前蜀高祖', start: 907, end: 918 },
    { name: '王衍', templeName: '前蜀后主', start: 918, end: 925 }
  ],
  TenKingdoms_LaterShu: [
    { name: '孟知祥', templeName: '后蜀高祖', start: 934, end: 934 },
    { name: '孟昶', templeName: '后蜀后主', start: 934, end: 965 }
  ],
  TenKingdoms_Jingnan: [
    { name: '高季兴', templeName: '南平武信王', start: 924, end: 928 },
    { name: '高从诲', templeName: '南平文献王', start: 928, end: 948 },
    { name: '高保融', templeName: '南平贞懿王', start: 948, end: 960 },
    { name: '高保勗', templeName: '南平贞安王', start: 960, end: 962 },
    { name: '高继冲', templeName: '南平德仁王', start: 962, end: 963 }
  ],
  TenKingdoms_SouthernTang: [
    { name: '李昪', templeName: '南唐烈祖', start: 937, end: 943 },
    { name: '李璟', templeName: '南唐元宗', start: 943, end: 961 },
    { name: '李煜', templeName: '南唐后主', start: 961, end: 975 }
  ],
  TenKingdoms_NorthernHan: [
    { name: '刘崇', templeName: '北汉世祖', start: 951, end: 954 },
    { name: '刘承钧', templeName: '北汉睿宗', start: 954, end: 968 },
    { name: '刘继恩', templeName: '北汉少主', start: 968, end: 968 },
    { name: '刘继元', templeName: '北汉英武帝', start: 968, end: 979 }
  ],
  Sui: [
    {
      name: '杨坚',
      templeName: '隋高祖',
      posthumousName: '文皇帝',
      era: ['开皇', '仁寿'],
      start: '581-03-04',
      end: '604-08-13',
      queens: [
        {
          name: '独孤伽罗',
          posthumousName: '文献皇后',
          start: '581',
          end: '602'
        }
      ]
    },
    {
      name: '杨广',
      templeName: '隋世祖',
      posthumousName: '炀皇帝',
      era: ['大业'],
      start: '604-08-21',
      end: '618-04-11',
      queens: [
        { name: '萧氏', posthumousName: '愍皇后', start: '605', end: '618' }
      ]
    }
  ],
  Tang: [
    {
      name: '李渊',
      templeName: '唐高祖',
      posthumousName: '神尧大圣大光孝皇帝',
      era: ['武德'],
      start: '618-06-18',
      end: '626-09-04',
      queens: []
    },
    {
      name: '李世民',
      templeName: '唐太宗',
      posthumousName: '文武大圣大广孝皇帝',
      era: ['贞观'],
      start: '626-09-04',
      end: '649-07-10',
      queens: [
        {
          name: '长孙氏',
          posthumousName: '文德皇后',
          start: '626-09',
          end: '636-07'
        }
      ]
    },
    {
      name: '李治',
      templeName: '唐高宗',
      posthumousName: '天皇大圣大弘孝皇帝',
      era: [
        '永徽',
        '显庆',
        '龙朔',
        '麟德',
        '乾封',
        '总章',
        '咸亨',
        '上元',
        '仪凤',
        '调露',
        '永隆',
        '开耀',
        '永淳',
        '弘道'
      ],
      start: '649-07-15',
      end: '683-12-27',
      queens: [
        { name: '王氏', posthumousName: '', start: '650-02', end: '655-11' },
        {
          name: '武曌',
          posthumousName: '则天大圣皇帝',
          start: '655-11',
          end: '690-10-16'
        }
      ]
    },
    {
      name: '李显',
      templeName: '唐中宗',
      posthumousName: '大和大圣大昭孝皇帝',
      era: ['嗣圣'],
      start: '684-01-03',
      end: '684-02-26',
      queens: [
        { name: '韦氏', posthumousName: '韦皇后', start: '705', end: '710' }
      ]
    },
    {
      name: '李旦',
      templeName: '唐睿宗',
      posthumousName: '玄真大圣大兴孝皇帝',
      era: ['文明'],
      start: '684-02-27',
      end: '690-10-16',
      queens: []
    },
    {
      name: '武曌',
      templeName: '',
      posthumousName: '则天大圣皇帝',
      era: [
        '光宅',
        '垂拱',
        '永昌',
        '载初',
        '天授',
        '如意',
        '长寿',
        '延载',
        '证圣',
        '天册万岁',
        '万岁登封',
        '万岁通天',
        '神功',
        '圣历',
        '久视',
        '大足',
        '长安',
        '神龙'
      ],
      start: '690-10-16',
      end: '705-02-22',
      queens: []
    },
    {
      name: '李显',
      templeName: '唐中宗(复辟)',
      posthumousName: '大和大圣大昭孝皇帝',
      era: ['神龙', '景龙'],
      start: '705-02-23',
      end: '710-07-03',
      queens: [
        { name: '韦氏', posthumousName: '韦皇后', start: '705', end: '710' }
      ]
    },
    {
      name: '李旦',
      templeName: '唐睿宗(复辟)',
      posthumousName: '玄真大圣大兴孝皇帝',
      era: ['景云', '太极', '延和'],
      start: '710-07-25',
      end: '712-09-08',
      queens: []
    },
    {
      name: '李隆基',
      templeName: '唐玄宗',
      posthumousName: '至道大圣大明孝皇帝',
      era: ['先天', '开元', '天宝'],
      start: '712-09-08',
      end: '756-08-12',
      queens: [
        {
          name: '王氏',
          posthumousName: '王皇后',
          start: '712-09',
          end: '724-08'
        }
      ]
    },
    {
      name: '李亨',
      templeName: '唐肃宗',
      posthumousName: '文明武德大圣大宣孝皇帝',
      era: ['至德', '乾元', '上元'],
      start: '756-08-12',
      end: '762-05-16',
      queens: [
        {
          name: '张氏',
          posthumousName: '张皇后',
          start: '758-05',
          end: '762-05'
        }
      ]
    },
    {
      name: '李豫',
      templeName: '唐代宗',
      posthumousName: '睿文孝武皇帝',
      era: ['宝应', '广德', '永泰', '大历'],
      start: '762-05-18',
      end: '779-06-10',
      queens: []
    },
    {
      name: '李适',
      templeName: '唐德宗',
      posthumousName: '神武孝文皇帝',
      era: ['建中', '兴元', '贞元'],
      start: '779-06-12',
      end: '805-02-11',
      queens: [
        { name: '王氏', posthumousName: '昭德皇后', start: '786', end: '786' }
      ]
    },
    {
      name: '李诵',
      templeName: '唐顺宗',
      posthumousName: '至德大圣大安孝皇帝',
      era: ['永贞'],
      start: '805-02-28',
      end: '805-08-31',
      queens: []
    },
    {
      name: '李纯',
      templeName: '唐宪宗',
      posthumousName: '昭文章武大圣至神孝皇帝',
      era: ['元和'],
      start: '805-09-05',
      end: '820-02-14',
      queens: []
    },
    {
      name: '李恒',
      templeName: '唐穆宗',
      posthumousName: '睿圣文惠孝皇帝',
      era: ['长庆'],
      start: '820-02-20',
      end: '824-02-25',
      queens: []
    },
    {
      name: '李湛',
      templeName: '唐敬宗',
      posthumousName: '睿武昭泯孝皇帝',
      era: ['宝历'],
      start: '824-02-29',
      end: '827-01-09',
      queens: []
    },
    {
      name: '李昂',
      templeName: '唐文宗',
      posthumousName: '元圣昭献孝皇帝',
      era: ['大和', '开成'],
      start: '827-01-13',
      end: '840-02-10',
      queens: []
    },
    {
      name: '李炎',
      templeName: '唐武宗',
      posthumousName: '至道昭肃孝皇帝',
      era: ['会昌'],
      start: '840-02-20',
      end: '846-04-22',
      queens: []
    },
    {
      name: '李忱',
      templeName: '唐宣宗',
      posthumousName: '圣武献文孝皇帝',
      era: ['大中'],
      start: '846-04-25',
      end: '859-09-07',
      queens: []
    },
    {
      name: '李漼',
      templeName: '唐懿宗',
      posthumousName: '昭圣恭惠孝皇帝',
      era: ['咸通'],
      start: '859-09-13',
      end: '873-08-15',
      queens: []
    },
    {
      name: '李儇',
      templeName: '唐僖宗',
      posthumousName: '惠圣恭定孝皇帝',
      era: ['乾符', '广明', '中和', '光启', '文德'],
      start: '873-08-16',
      end: '888-04-20',
      queens: []
    },
    {
      name: '李晔',
      templeName: '唐昭宗',
      posthumousName: '圣穆景文孝皇帝',
      era: ['龙纪', '大顺', '景福', '乾宁', '光化', '天复', '天祐'],
      start: '888-04-20',
      end: '904-09-22',
      queens: []
    },
    {
      name: '李柷',
      templeName: '唐哀帝',
      posthumousName: '昭宣光烈孝皇帝',
      era: ['天祐'],
      start: '904-09-26',
      end: '907-05-12',
      queens: []
    }
  ],
  NorthernSong: [
    {
      name: '赵匡胤',
      templeName: '宋太祖',
      posthumousName: '英武圣文神德皇帝',
      era: ['建隆', '乾德', '开宝'],
      start: '960-02-04',
      end: '976-11-14',
      queens: [
        {
          name: '王氏',
          posthumousName: '孝明皇后',
          start: '960-02',
          end: '963-12'
        },
        {
          name: '宋氏',
          posthumousName: '孝章皇后',
          start: '968-02',
          end: '995-06'
        }
      ]
    },
    {
      name: '赵光义',
      templeName: '宋太宗',
      posthumousName: '神功圣德文武皇帝',
      era: ['太平兴国', '雍熙', '端拱', '淳化', '至道'],
      start: '976-11-15',
      end: '997-05-08',
      queens: [
        {
          name: '李氏',
          posthumousName: '明德皇后',
          start: '984-12',
          end: '1004-04'
        }
      ]
    },
    {
      name: '赵恒',
      templeName: '宋真宗',
      posthumousName: '文明武定章圣元孝皇帝',
      era: ['咸平', '景德', '大中祥符', '天禧', '乾兴'],
      start: '997-05-10',
      end: '1022-03-23',
      queens: [
        {
          name: '郭氏',
          posthumousName: '章穆皇后',
          start: '997-05',
          end: '1007-05'
        },
        {
          name: '刘氏',
          posthumousName: '章献明肃皇后',
          start: '1012-12',
          end: '1033-04'
        }
      ]
    },
    {
      name: '赵祯',
      templeName: '宋仁宗',
      posthumousName: '神文圣武明孝皇帝',
      era: [
        '天圣',
        '明道',
        '景祐',
        '宝元',
        '康定',
        '庆历',
        '皇祐',
        '至和',
        '嘉祐'
      ],
      start: '1022-03-24',
      end: '1063-04-30',
      queens: [
        { name: '郭氏', posthumousName: '', start: '1024-12', end: '1033-12' },
        {
          name: '曹氏',
          posthumousName: '慈圣光献皇后',
          start: '1034-09',
          end: '1079-11'
        }
      ]
    },
    {
      name: '赵曙',
      templeName: '宋英宗',
      posthumousName: '宪文肃武宣孝皇帝',
      era: ['治平'],
      start: '1063-05-01',
      end: '1067-01-25',
      queens: [
        {
          name: '高氏',
          posthumousName: '宣仁圣烈皇后',
          start: '1063',
          end: '1093'
        }
      ]
    },
    {
      name: '赵顼',
      templeName: '宋神宗',
      posthumousName: '英文烈武圣孝皇帝',
      era: ['熙宁', '元丰'],
      start: '1067-01-26',
      end: '1085-04-01',
      queens: [
        {
          name: '向氏',
          posthumousName: '钦圣宪肃皇后',
          start: '1068',
          end: '1101'
        }
      ]
    },
    {
      name: '赵煦',
      templeName: '宋哲宗',
      posthumousName: '钦文睿武昭孝皇帝',
      era: ['元祐', '绍圣', '元符'],
      start: '1085-04-01',
      end: '1100-02-23',
      queens: [
        {
          name: '孟氏',
          posthumousName: '昭慈圣献皇后',
          start: '1092',
          end: '1096'
        },
        { name: '刘氏', posthumousName: '昭怀皇后', start: '1099', end: '1113' }
      ]
    },
    {
      name: '赵佶',
      templeName: '宋徽宗',
      posthumousName: '圣文仁德显孝皇帝',
      era: ['建中靖国', '崇宁', '大观', '政和', '重和', '宣和'],
      start: '1100-02-23',
      end: '1126-01-18',
      queens: [
        {
          name: '王氏',
          posthumousName: '显恭皇后',
          start: '1100',
          end: '1108'
        },
        { name: '郑氏', posthumousName: '显肃皇后', start: '1111', end: '1131' }
      ]
    },
    {
      name: '赵桓',
      templeName: '宋钦宗',
      posthumousName: '恭文顺德仁孝皇帝',
      era: ['靖康'],
      start: '1126-01-19',
      end: '1127-03-20',
      queens: [
        { name: '朱氏', posthumousName: '仁怀皇后', start: '1126', end: '1127' }
      ]
    }
  ],
  SouthernSong: [
    {
      name: '赵构',
      templeName: '宋高宗',
      posthumousName: '受命中兴全功至德圣神武文昭仁宪孝皇帝',
      era: ['建炎', '绍兴'],
      start: '1127-06-12',
      end: '1162-07-24',
      queens: [
        {
          name: '邢氏',
          posthumousName: '懿节皇后',
          start: '1127',
          end: '1139'
        },
        {
          name: '吴氏',
          posthumousName: '宪圣慈烈皇后',
          start: '1143',
          end: '1197'
        }
      ]
    },
    {
      name: '赵昚',
      templeName: '宋孝宗',
      posthumousName: '成孝皇帝',
      era: ['隆兴', '乾道', '淳熙'],
      start: '1162-07-24',
      end: '1189-02-18',
      queens: [
        {
          name: '成氏',
          posthumousName: '恭夏皇后',
          start: '1162',
          end: '1167'
        },
        { name: '谢氏', posthumousName: '成肃皇后', start: '1167', end: '1207' }
      ]
    },
    {
      name: '赵惇',
      templeName: '宋光宗',
      posthumousName: '慈孝皇帝',
      era: ['绍熙'],
      start: '1189-02-18',
      end: '1194-07-24',
      queens: [
        { name: '李氏', posthumousName: '慈懿皇后', start: '1189', end: '1200' }
      ]
    },
    {
      name: '赵扩',
      templeName: '宋宁宗',
      posthumousName: '恭孝皇帝',
      era: ['庆元', '嘉泰', '开禧', '嘉定'],
      start: '1194-07-24',
      end: '1224-09-17',
      queens: [
        {
          name: '韩氏',
          posthumousName: '恭淑皇后',
          start: '1194',
          end: '1200'
        },
        {
          name: '杨氏',
          posthumousName: '恭圣仁烈皇后',
          start: '1202',
          end: '1232'
        }
      ]
    },
    {
      name: '赵昀',
      templeName: '宋理宗',
      posthumousName: '安孝皇帝',
      era: ['宝庆', '绍定', '端平', '嘉熙', '淳祐', '宝祐', '开庆', '景定'],
      start: '1224-09-17',
      end: '1264-11-16',
      queens: [
        {
          name: '谢道清',
          posthumousName: '寿和圣福皇后',
          start: '1227',
          end: '1283'
        }
      ]
    },
    {
      name: '赵禥',
      templeName: '宋度宗',
      posthumousName: '景孝皇帝',
      era: ['咸淳'],
      start: '1264-11-16',
      end: '1274-08-12',
      queens: [
        { name: '全氏', posthumousName: '全皇后', start: '1267', end: '1276' }
      ]
    },
    {
      name: '赵显',
      templeName: '宋恭帝',
      posthumousName: '恭皇帝',
      era: ['德祐'],
      start: '1274-08-12',
      end: '1276-02-04',
      queens: []
    },
    {
      name: '赵昰',
      templeName: '宋端宗',
      posthumousName: '愍孝皇帝',
      era: ['景炎'],
      start: '1276-06-14',
      end: '1278-05-08',
      queens: []
    },
    {
      name: '赵昺',
      templeName: '宋怀宗',
      posthumousName: '末帝',
      era: ['祥兴'],
      start: '1278-05-10',
      end: '1279-03-19',
      queens: []
    }
  ],
  Yuan: [
    {
      name: '忽必烈',
      templeName: '元世祖',
      posthumousName: '圣德神功文武皇帝',
      era: ['中统', '至元'],
      start: '1271-12-18',
      end: '1294-02-18',
      queens: []
    },
    {
      name: '铁穆耳',
      templeName: '元成宗',
      posthumousName: '钦明广孝皇帝',
      era: ['元贞', '大德'],
      start: '1294-05-10',
      end: '1307-02-10',
      queens: []
    },
    {
      name: '海山',
      templeName: '元武宗',
      posthumousName: '仁惠宣孝皇帝',
      era: ['至大'],
      start: '1307-06-21',
      end: '1311-01-27',
      queens: []
    },
    {
      name: '爱育黎拔力八达',
      templeName: '元仁宗',
      posthumousName: '圣文钦孝皇帝',
      era: ['皇庆', '延祐'],
      start: '1311-04-07',
      end: '1320-03-01',
      queens: []
    },
    {
      name: '硕德八刺',
      templeName: '元英宗',
      posthumousName: '睿圣文孝皇帝',
      era: ['至治'],
      start: '1320-04-19',
      end: '1323-09-04',
      queens: []
    },
    {
      name: '也孙铁木儿',
      templeName: '元泰定帝',
      posthumousName: '',
      era: ['泰定', '致和'],
      start: '1323-10-04',
      end: '1328-08-15',
      queens: []
    },
    {
      name: '阿速吉八',
      templeName: '元天顺帝',
      posthumousName: '',
      era: ['天顺'],
      start: '1328-10-01',
      end: '1328-11-14',
      queens: []
    },
    {
      name: '图帖睦尔',
      templeName: '元文宗',
      posthumousName: '圣明元孝皇帝',
      era: ['天历', '至顺'],
      start: '1328-10-16',
      end: '1329-02-26',
      queens: []
    },
    {
      name: '和世琜',
      templeName: '元明宗',
      posthumousName: '翼献景孝皇帝',
      era: ['天历'],
      start: '1329-02-27',
      end: '1329-08-30',
      queens: []
    },
    {
      name: '图帖睦尔',
      templeName: '元文宗(复位)',
      posthumousName: '圣明元孝皇帝',
      era: ['天历', '至顺'],
      start: '1329-09-08',
      end: '1332-09-02',
      queens: []
    },
    {
      name: '懿璘质班',
      templeName: '元宁宗',
      posthumousName: '冲圣嗣孝皇帝',
      era: ['至顺'],
      start: '1332-10-23',
      end: '1332-12-14',
      queens: []
    },
    {
      name: '妥懽帖睦尔',
      templeName: '元惠宗',
      posthumousName: '顺皇帝',
      era: ['元统', '至元', '至正'],
      start: '1333-07-19',
      end: '1368-09-14',
      queens: []
    }
  ],
  Ming: [
    {
      name: '朱元璋',
      templeName: '明太祖',
      posthumousName: '高皇帝',
      era: ['洪武'],
      start: '1368-01-23',
      end: '1398-06-24',
      queens: [
        {
          name: '马氏',
          posthumousName: '孝慈高皇后',
          start: '1368-01-23',
          end: '1382-09-17'
        }
      ]
    },
    {
      name: '朱允炆',
      templeName: '明惠宗',
      posthumousName: '让皇帝',
      era: ['建文'],
      start: '1398-06-30',
      end: '1402-07-13',
      queens: []
    },
    {
      name: '朱棣',
      templeName: '明成祖',
      posthumousName: '文皇帝',
      era: ['永乐'],
      start: '1402-07-17',
      end: '1424-08-12',
      queens: [
        {
          name: '徐氏',
          posthumousName: '仁孝文皇后',
          start: '1402-12',
          end: '1407-07'
        }
      ]
    },
    {
      name: '朱高炽',
      templeName: '明仁宗',
      posthumousName: '昭皇帝',
      era: ['洪熙'],
      start: '1424-09-07',
      end: '1425-05-29',
      queens: []
    },
    {
      name: '朱瞻基',
      templeName: '明宣宗',
      posthumousName: '章皇帝',
      era: ['宣德'],
      start: '1425-06-27',
      end: '1435-01-31',
      queens: [
        {
          name: '胡善祥',
          posthumousName: '恭让章皇后',
          start: '1425-08',
          end: '1428-03'
        },
        {
          name: '孙氏',
          posthumousName: '孝恭章皇后',
          start: '1428-03',
          end: '1462-09'
        }
      ]
    },
    {
      name: '朱祁镇',
      templeName: '明英宗',
      posthumousName: '睿皇帝',
      era: ['正统'],
      start: '1435-02-07',
      end: '1449-09-01',
      queens: [
        {
          name: '钱氏',
          posthumousName: '孝庄睿皇后',
          start: '1442-06',
          end: '1468-07'
        }
      ]
    },
    {
      name: '朱祁钰',
      templeName: '明代宗',
      posthumousName: '景皇帝',
      era: ['景泰'],
      start: '1449-09-22',
      end: '1457-02-11',
      queens: [
        {
          name: '汪氏',
          posthumousName: '孝渊景皇后',
          start: '1449',
          end: '1452'
        },
        { name: '杭氏', posthumousName: '肃孝皇后', start: '1452', end: '1456' }
      ]
    },
    {
      name: '朱祁镇',
      templeName: '明英宗(复辟)',
      posthumousName: '睿皇帝',
      era: ['天顺'],
      start: '1457-02-11',
      end: '1464-02-23',
      queens: []
    },
    {
      name: '朱见深',
      templeName: '明宪宗',
      posthumousName: '纯皇帝',
      era: ['成化'],
      start: '1464-02-28',
      end: '1487-09-09',
      queens: [
        { name: '吴氏', posthumousName: '', start: '1464', end: '1464' },
        {
          name: '王氏',
          posthumousName: '孝贞纯皇后',
          start: '1464',
          end: '1518'
        }
      ]
    },
    {
      name: '朱祐樘',
      templeName: '明孝宗',
      posthumousName: '敬皇帝',
      era: ['弘治'],
      start: '1487-09-22',
      end: '1505-06-08',
      queens: [
        {
          name: '张氏',
          posthumousName: '孝成敬皇后',
          start: '1487',
          end: '1541'
        }
      ]
    },
    {
      name: '朱厚照',
      templeName: '明武宗',
      posthumousName: '毅皇帝',
      era: ['正德'],
      start: '1505-06-19',
      end: '1521-04-20',
      queens: [
        {
          name: '夏氏',
          posthumousName: '孝静毅皇后',
          start: '1506',
          end: '1535'
        }
      ]
    },
    {
      name: '朱厚熜',
      templeName: '明世宗',
      posthumousName: '肃皇帝',
      era: ['嘉靖'],
      start: '1521-05-27',
      end: '1567-01-23',
      queens: [
        {
          name: '陈氏',
          posthumousName: '孝洁肃皇后',
          start: '1522',
          end: '1528'
        },
        { name: '张氏', posthumousName: '', start: '1529', end: '1534' },
        { name: '方氏', posthumousName: '孝烈皇后', start: '1534', end: '1547' }
      ]
    },
    {
      name: '朱载坖',
      templeName: '明穆宗',
      posthumousName: '庄皇帝',
      era: ['隆庆'],
      start: '1567-02-04',
      end: '1572-07-05',
      queens: [
        { name: '陈氏', posthumousName: '孝安皇后', start: '1567', end: '1596' }
      ]
    },
    {
      name: '朱翊钧',
      templeName: '明神宗',
      posthumousName: '显皇帝',
      era: ['万历'],
      start: '1572-07-19',
      end: '1620-08-18',
      queens: [
        {
          name: '王喜姐',
          posthumousName: '孝端显皇后',
          start: '1578',
          end: '1620'
        }
      ]
    },
    {
      name: '朱常洛',
      templeName: '明光宗',
      posthumousName: '贞皇帝',
      era: ['泰昌'],
      start: '1620-08-28',
      end: '1620-09-26',
      queens: []
    },
    {
      name: '朱由校',
      templeName: '明熹宗',
      posthumousName: '悊皇帝',
      era: ['天启'],
      start: '1620-10-01',
      end: '1627-09-30',
      queens: [
        {
          name: '张嫣',
          posthumousName: '孝哀悊皇后',
          start: '1621',
          end: '1644'
        }
      ]
    },
    {
      name: '朱由检',
      templeName: '明思宗',
      posthumousName: '烈皇帝',
      era: ['崇祯'],
      start: '1627-10-02',
      end: '1644-04-25',
      queens: [
        {
          name: '周氏',
          posthumousName: '孝节烈皇后',
          start: '1628',
          end: '1644'
        }
      ]
    }
  ],
  HouJin: [
    {
      name: '努尔哈赤',
      templeName: '‌清太祖',
      posthumousName: '高皇帝',
      era: [],
      start: 1616,
      end: 1626,
      queens: []
    },
    {
      name: '皇太极',
      templeName: '清太宗',
      posthumousName: '文皇帝',
      era: ['天聪'],
      start: 1626,
      end: 1636,
      queens: []
    }
  ],
  Qing: [
    {
      name: '皇太极',
      templeName: '清太宗',
      posthumousName: '文皇帝',
      era: ['崇德'],
      start: 1636,
      end: '1643-08',
      queens: [
        {
          name: '哲哲',
          posthumousName: '孝端文皇后',
          start: '1636-04',
          end: '1649-04-17'
        },
        {
          name: '布木布泰',
          posthumousName: '庄妃（孝庄文皇后）',
          start: '1636-04',
          end: '1687-12-26'
        }
      ]
    },
    {
      name: '福临',
      templeName: '清世祖',
      posthumousName: '章皇帝',
      era: ['顺治'],
      start: '1643-08',
      end: '1661-02-05',
      queens: [
        {
          name: '博尔济吉特氏',
          posthumousName: '静妃',
          start: '1651-09',
          end: '1653-10'
        },
        {
          name: '博尔济吉特氏',
          posthumousName: '孝惠章皇后',
          start: '1654-06',
          end: '1718-01'
        }
      ]
    },
    {
      name: '玄烨',
      templeName: '清圣祖',
      posthumousName: '仁皇帝',
      era: ['康熙'],
      start: '1661-02-07',
      end: '1722-12-20',
      queens: [
        {
          name: '赫舍里氏',
          posthumousName: '孝诚仁皇后',
          start: '1665-10',
          end: '1674-06'
        },
        {
          name: '钮祜禄氏',
          posthumousName: '孝昭仁皇后',
          start: '1677-09',
          end: '1678-03'
        },
        {
          name: '佟佳氏',
          posthumousName: '孝懿仁皇后',
          start: '1689-08-23',
          end: '1689-08-24'
        }
      ]
    },
    {
      name: '胤禛',
      templeName: '清世宗',
      posthumousName: '宪皇帝',
      era: ['雍正'],
      start: '1722-12-27',
      end: '1735-10-08',
      queens: [
        {
          name: '乌拉那拉氏',
          posthumousName: '孝敬宪皇后',
          start: '1722-12',
          end: '1731-10'
        }
      ]
    },
    {
      name: '弘历',
      templeName: '清高宗',
      posthumousName: '纯皇帝',
      era: ['乾隆'],
      start: '1735-10-18',
      end: '1796-02-08',
      queens: [
        {
          name: '富察氏',
          posthumousName: '孝贤纯皇后',
          start: '1737-12-04',
          end: '1748-04-08'
        },
        {
          name: '那拉氏',
          posthumousName: '继皇后',
          start: '1750-09-02',
          end: '1766-08-19'
        }
      ]
    },
    {
      name: '颙琰',
      templeName: '清仁宗',
      posthumousName: '睿皇帝',
      era: ['嘉庆'],
      start: '1796-02',
      end: '1820-09-02',
      queens: [
        {
          name: '喜塔腊氏',
          posthumousName: '孝淑睿皇后',
          start: '1796-02',
          end: '1797-03'
        },
        {
          name: '钮祜禄氏',
          posthumousName: '孝和睿皇后',
          start: '1801-05',
          end: '1850-01'
        }
      ]
    },
    {
      name: '旻宁',
      templeName: '清宣宗',
      posthumousName: '成皇帝',
      era: ['道光'],
      start: '1820-10-03',
      end: '1850-02-25',
      queens: [
        {
          name: '钮祜禄氏',
          posthumousName: '孝慎成皇后',
          start: '1820-12',
          end: '1833-06'
        },
        {
          name: '钮祜禄氏',
          posthumousName: '孝全成皇后',
          start: '1834-11',
          end: '1840-02'
        }
      ]
    },
    {
      name: '奕詝',
      templeName: '清文宗',
      posthumousName: '显皇帝',
      era: ['咸丰'],
      start: '1850-03-09',
      end: '1861-08-22',
      queens: [
        {
          name: '钮祜禄氏',
          posthumousName: '孝贞显皇后',
          start: '1852-10',
          end: '1881-04-08'
        }
      ]
    },
    {
      name: '载淳',
      templeName: '清穆宗',
      posthumousName: '毅皇帝',
      era: ['同治'],
      start: '1861-11-11',
      end: '1875-01-12',
      queens: [
        {
          name: '阿鲁特氏',
          posthumousName: '孝哲毅皇后',
          start: '1872-10',
          end: '1875-03'
        }
      ]
    },
    {
      name: '载湉',
      templeName: '清德宗',
      posthumousName: '景皇帝',
      era: ['光绪'],
      start: '1875-02-25',
      end: '1908-11-14',
      queens: [
        {
          name: '叶赫那拉氏',
          posthumousName: '孝定景皇后',
          start: '1889-02-26',
          end: '1913-02-22'
        }
      ]
    },
    {
      name: '溥仪',
      templeName: '清逊帝',
      posthumousName: '宣统帝',
      era: ['宣统'],
      start: '1908-12-02',
      end: '1912-02-12',
      queens: [
        {
          name: '郭布罗·婉容',
          posthumousName: '孝恪愍皇后',
          start: '1922-12-01',
          end: '1946-06-20'
        }
      ]
    }
  ],
  Liao: [
    {
      name: '耶律阿保机',
      templeName: '辽太祖',
      posthumousName: '大圣大明神烈天皇帝',
      era: ['神册', '天赞', '天显'],
      start: 916,
      end: 926,
      queens: []
    },
    {
      name: '耶律德光',
      templeName: '辽太宗',
      posthumousName: '孝武惠文皇帝',
      era: ['天显', '会同', '大同'],
      start: 927,
      end: 947,
      queens: []
    },
    {
      name: '耶律阮',
      templeName: '辽世宗',
      posthumousName: '孝和庄宪皇帝',
      era: ['天禄'],
      start: 947,
      end: 951,
      queens: []
    },
    {
      name: '耶律璟',
      templeName: '辽穆宗',
      posthumousName: '孝安敬正皇帝',
      era: ['应历'],
      start: 951,
      end: 969,
      queens: []
    },
    {
      name: '耶律贤',
      templeName: '辽景宗',
      posthumousName: '孝成康靖皇帝',
      era: ['保宁', '乾亨'],
      start: 969,
      end: 982,
      queens: []
    },
    {
      name: '耶律隆绪',
      templeName: '辽圣宗',
      posthumousName: '文武大孝宣皇帝',
      era: ['统和', '开泰', '太平'],
      start: 982,
      end: 1031,
      queens: []
    },
    {
      name: '耶律宗真',
      templeName: '辽兴宗',
      posthumousName: '神圣孝章皇帝',
      era: ['景福', '重熙'],
      start: 1031,
      end: 1055,
      queens: []
    },
    {
      name: '耶律洪基',
      templeName: '辽道宗',
      posthumousName: '仁圣大孝文皇帝',
      era: ['清宁', '咸雍', '大康', '大安', '寿昌'],
      start: 1055,
      end: 1101,
      queens: []
    },
    {
      name: '耶律延禧',
      templeName: '',
      posthumousName: '天祚皇帝',
      era: ['乾统', '天庆', '保大'],
      start: 1101,
      end: 1125,
      queens: []
    }
  ],
  WesternXia: [
    {
      name: '李元昊',
      templeName: '夏景宗',
      posthumousName: '武烈皇帝',
      era: ['显道', '开运', '广运', '大庆', '天授礼法延祚'],
      start: 1038,
      end: 1048,
      queens: []
    },
    {
      name: '李谅祚',
      templeName: '夏毅宗',
      posthumousName: '昭英皇帝',
      era: ['延嗣宁国', '天祐垂圣', '福圣承道', '奲都', '拱化'],
      start: 1048,
      end: 1067,
      queens: []
    },
    {
      name: '李秉常',
      templeName: '夏惠宗',
      posthumousName: '康靖皇帝',
      era: ['乾道', '天赐礼盛国庆', '大安', '天安礼定', '天仪治平'],
      start: 1067,
      end: 1086,
      queens: []
    },
    {
      name: '李乾顺',
      templeName: '夏崇宗',
      posthumousName: '圣文皇帝',
      era: ['天祐民安', '永安', '贞观', '雍宁', '元德', '正德', '大德'],
      start: 1086,
      end: 1139,
      queens: []
    },
    {
      name: '李仁孝',
      templeName: '夏仁宗',
      posthumousName: '圣德皇帝',
      era: ['大庆', '人庆', '天盛', '乾祐'],
      start: 1139,
      end: 1193,
      queens: []
    },
    {
      name: '李纯佑',
      templeName: '夏桓宗',
      posthumousName: '昭简皇帝',
      era: ['天庆'],
      start: 1193,
      end: 1206,
      queens: []
    },
    {
      name: '李安全',
      templeName: '夏襄宗',
      posthumousName: '敬穆皇帝',
      era: ['应天', '皇建'],
      start: 1206,
      end: 1211,
      queens: []
    },
    {
      name: '李遵顼',
      templeName: '夏神宗',
      posthumousName: '英文皇帝',
      era: ['光定'],
      start: 1211,
      end: 1223,
      queens: []
    },
    {
      name: '李德旺',
      templeName: '夏献宗',
      posthumousName: '孝哀皇帝',
      era: ['乾定'],
      start: 1223,
      end: 1226,
      queens: []
    },
    {
      name: '李睍',
      templeName: '',
      posthumousName: '末帝',
      era: ['宝义'],
      start: 1226,
      end: 1227,
      queens: []
    }
  ],
  Jin: [
    {
      name: '完颜阿骨打',
      templeName: '金太祖',
      posthumousName: '应乾兴运昭德定功仁明庄孝大圣武元皇帝',
      era: ['收国', '天辅'],
      start: 1115,
      end: 1123,
      queens: []
    },
    {
      name: '完颜吴乞买',
      templeName: '金太宗',
      posthumousName: '体元应运世德昭功哲惠仁圣文烈皇帝',
      era: ['天会'],
      start: 1123,
      end: 1135,
      queens: []
    },
    {
      name: '完颜亶',
      templeName: '金熙宗',
      posthumousName: '弘基缵武庄靖孝成皇帝',
      era: ['天会', '天眷', '皇统'],
      start: 1135,
      end: 1150,
      queens: []
    },
    {
      name: '完颜亮',
      templeName: '',
      posthumousName: '海陵王',
      era: ['天德', '贞元', '正隆'],
      start: 1150,
      end: 1161,
      queens: []
    },
    {
      name: '完颜雍',
      templeName: '金世宗',
      posthumousName: '光天兴运文德武功圣明仁孝皇帝',
      era: ['大定'],
      start: 1161,
      end: 1189,
      queens: []
    },
    {
      name: '完颜璟',
      templeName: '金章宗',
      posthumousName: '宪天光运仁文义武神圣英孝皇帝',
      era: ['明昌', '承安', '泰和'],
      start: 1189,
      end: 1208,
      queens: []
    },
    {
      name: '完颜永济',
      templeName: '',
      posthumousName: '卫绍王',
      era: ['大安', '崇庆', '至宁'],
      start: 1208,
      end: 1213,
      queens: []
    },
    {
      name: '完颜珣',
      templeName: '金宣宗',
      posthumousName: '继天兴统述道勤仁英武圣孝皇帝',
      era: ['贞祐', '兴定', '元光'],
      start: 1213,
      end: 1224,
      queens: []
    },
    {
      name: '完颜守绪',
      templeName: '金哀宗',
      posthumousName: '敬天德运忠文靖武天圣烈孝庄皇帝',
      era: ['正大', '开兴', '天兴'],
      start: 1224,
      end: 1234,
      queens: []
    }
  ],
  ROC: [
    // 南京临时政府
    {
      name: '孙中山',
      templeName: '南京临时政府-临时大总统',
      posthumousName: '国父',
      start: '1912-01-01',
      end: '1912-04-01'
    },
    // 北洋政府
    {
      name: '袁世凯',
      templeName: '北洋政府-正式总统',
      posthumousName: '',
      start: '1912-03-10',
      end: '1916-06-06'
    },
    {
      name: '黎元洪',
      templeName: '北洋政府-第一任总统',
      posthumousName: '',
      start: '1916-06-07',
      end: '1917-07-01'
    },
    {
      name: '冯国璋',
      templeName: '北洋政府-代理总统',
      posthumousName: '',
      start: '1917-07-06',
      end: '1918-10-10'
    },
    {
      name: '徐世昌',
      templeName: '北洋政府-总统',
      posthumousName: '',
      start: '1918-10-10',
      end: '1922-06-02'
    },
    {
      name: '周自齐',
      templeName: '北洋政府-国务总理摄行总统职权',
      posthumousName: '',
      start: '1922-06-02',
      end: '1922-06-11'
    },
    {
      name: '黎元洪',
      templeName: '北洋政府-第二任总统',
      posthumousName: '',
      start: '1922-06-11',
      end: '1923-06-13'
    },
    {
      name: '高凌霨',
      templeName: '北洋政府-国务总理摄行总统职权',
      posthumousName: '',
      start: '1923-06-14',
      end: '1923-10-10'
    },
    {
      name: '曹锟',
      templeName: '北洋政府-总统',
      posthumousName: '',
      start: '1923-10-10',
      end: '1924-11-02'
    },
    {
      name: '黄郛',
      templeName: '北洋政府-国务总理摄行总统职权',
      posthumousName: '',
      start: '1924-11-02',
      end: '1924-11-23'
    },
    {
      name: '段祺瑞',
      templeName: '北洋政府-临时执政',
      posthumousName: '',
      start: '1924-11-24',
      end: '1926-04-20'
    },
    {
      name: '胡惟德/颜惠庆/杜钖圭/顾维钧',
      templeName: '北洋政府-国务总理代行职权',
      posthumousName: '',
      start: '1926-04-20',
      end: '1927-06-16'
    },
    {
      name: '张作霖',
      templeName: '北洋政府-陆海军大元帅',
      posthumousName: '',
      start: '1927-06-18',
      end: '1928-06-03'
    },
    // 国民政府
    {
      name: '汪精卫',
      templeName: '国民政府-国民政府主席',
      posthumousName: '',
      start: '1925-07-01',
      end: '1926-03-20'
    },
    {
      name: '谭延闿',
      templeName: '国民政府-国民政府主席',
      posthumousName: '',
      start: '1926-03-20',
      end: '1927-03-12'
    },
    {
      name: '胡汉民',
      templeName: '国民政府-国民政府主席',
      posthumousName: '',
      start: '1927-04-18',
      end: '1927-09-13'
    },
    {
      name: '集体执政',
      templeName: '国民政府-胡汉民、谭延闿、孙科、徐谦、宋子文集体主持政务',
      posthumousName: '',
      start: '1927-07-15',
      end: '1928-02-06'
    },
    {
      name: '谭延闿',
      templeName: '国民政府-国民政府主席',
      posthumousName: '',
      start: '1928-02-06',
      end: '1928-10-10'
    },
    {
      name: '蒋介石',
      templeName: '国民政府-国民政府主席',
      posthumousName: '',
      start: '1928-10-10',
      end: '1931-12-15'
    },
    {
      name: '林森',
      templeName: '国民政府-国民政府主席',
      posthumousName: '',
      start: '1931-12-15',
      end: '1943-08-01'
    },
    {
      name: '蒋介石',
      templeName: '国民政府-军事委员会委员长',
      posthumousName: '',
      start: '1931-12-15',
      end: '1943-08-01'
    },
    {
      name: '蒋介石',
      templeName: '国民政府-国民政府主席',
      posthumousName: '',
      start: '1943-08-01',
      end: '1948-05-20'
    },
    // 行宪后
    {
      name: '蒋介石',
      templeName: '国民政府-总统',
      posthumousName: '',
      start: '1948-05-20',
      end: '1949-01-21'
    },
    {
      name: '李宗仁',
      templeName: '国民政府-代总统',
      posthumousName: '',
      start: '1949-01-21',
      end: '1949-10-01'
    }
  ]
}
